...
- 第一版后端也已经废弃。
- 新前端只能对接 CMS V2 后端。
- 旧 V1 UI 文档和旧静态页面只作为历史参考，不再作为当前实现基础。

当前阶段 1 只做文档清理，不创建前端工程，不开始页面实现。
...
- `frontend-v2/` 是当前建议目录名。
- 若后续确定使用其他目录名或项目名，也必须保持“独立 V2 工程 + 只对接 `/api/cms-v2`”这一原则。
...
- 在用户明确要求视觉精修前，默认使用最简样式，不自行补充装饰性视觉。
- 优先使用 shadcn-vue 与 Tailwind token，不在组件中散落一次性颜色值。
...
## 5.1 Server-confirmed Update �߽�

��ǰ V2 ǰ�˲��� server-confirmed update ��Ϊ�־û�����ԭ��
